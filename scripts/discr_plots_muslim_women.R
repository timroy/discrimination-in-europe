...
# Subset for women
for(i in 1:length(sv)) {
  sv_etmg[[i]] <- filter(sv[[i]], gndr == 2) #  women
}

sv_etmg_men <- list()
# Subset for men
for(i in 1:length(sv)) {
  sv_etmg_men[[i]] <- filter(sv[[i]], gndr == 2) #  women
}
...
# # subset for minorities
# for(i in 1:length(sv)) {
#   sv[[i]] <- filter(sv[[i]], blgetmg == 1)
# }
...
france <- list()
#for(i in 1:length()) { # for ethnic minorities
...
plot_round_paths <- c("./discr_figs/france_by_round_rlg_women.gif",
                      "./discr_figs/england_by_round_rlg_women.gif")
# set outcome animation paths
plot_outcome_paths <- c("./discr_figs/france_by_outcome_rlg_women.gif",
                        "./discr_figs/england_by_outcome_rlg_women.gif")
...
for(i in 1:length(df_list)) {
  ...
  plot_list[[i]] <- 
    ...
    labs(title = "Proportion of Muslim Women Discriminated Against", 
