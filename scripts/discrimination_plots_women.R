...
for(i in 1:length(sv)) {
  sv_etmg[[i]] <- filter(sv[[i]], blgetmg == 1) # minority 
}
...
plot_round_paths <- c("./discr_figs/france_by_round_women.gif",
                      "./discr_figs/england_by_round_women.gif")
# set outcome animation paths
plot_outcome_paths <- c("./discr_figs/france_by_outcome_women.gif",
                        "./discr_figs/england_by_outcome_women.gif")
...
for(i in 1:length(df_list)) {
  ...
  plot_list[[i]] <- 
    ...
    labs(title = "Proportion of Minority Women Discriminated Against", 
